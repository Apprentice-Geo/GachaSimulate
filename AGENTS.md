...
- 修改 CLI 或保存结果时，确认启动请求选择 result item、Compiler 写入对应 IR 索引、GSR v2、Analysis 和独立 DisplayConfig `*.visualize.json` sidecar 契约，并验证 Electron 与 Remotion 导出仍能消费 `Analysis + DisplayConfig`。
