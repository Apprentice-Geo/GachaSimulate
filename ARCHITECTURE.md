...
- `src/main/`：受信任的 Electron 宿主；`SimulationTask` 管理 core 与模拟产物，`ResultEditor` 管理 analyzer、带身份的结果会话与导出快照，`ExportTaskCoordinator` 管理导出 reservation、main-owned 目录选择、目标文件身份和统一准入，内部 `ExportHost` 管理隐藏导出窗口、CDP 截图、FFmpeg 与逐产物安全提交。
