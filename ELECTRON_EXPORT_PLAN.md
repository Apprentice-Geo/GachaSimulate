...
- 使用最终安装产物检查 MP4/PNG、覆盖、空格及中文路径、字体和视觉一致性；断网且无开发工具或 PATH 依赖时仍能导出。
- 测量空闲、模拟和分析期间的耗时、响应、内存与系统稳定性，由维护者决定继续、优化或更换路线。
...
2. 用单一人工归档替代 Phase 0 原报告目录，保留环境、版本/哈希、正确性、性能、故障清理和路线决策依据。
3. 删除 Remotion 导出宿主、相关依赖与传递打包产物，更新 lockfile、构建、CI 和文档。
4. 重新构建开发态与 Windows 安装包，复跑产物、连续帧、生命周期和断网检查。
