...
`gachasimulate_result` 同时维护 writer 和严格 reader。reader 在按声明分配数组前校验 header 与资源上限，随后只读取所选的 draw 或 cost section、termination IDs 和 reason string table；它还校验所选 metric 的 header total。统计输出契约见 [Analysis JSON v1](ANALYSIS_V1.md)。
