...
- 顶部标题区高度：96px；
- 主内容区高度：760px；
- 主内容区域左右布局，左侧 CDF 图表区域约 75%，右侧统计面板区域约 25%；
- 底部说明区高度：140px。
...
- X 轴表示抽数，显示为整数。
- Y 轴表示累计概率 CDF，显示为百分比，并在图表左侧显示竖排轴标签 `累计概率 CDF`。
- 坐标轴刻度、轴线和 tick line 必须在深色背景上清晰可读；当前实现使用浅色弱对比文字和 hairline 轴线。
- CDF 图使用横向和纵向低对比虚线网格，网格必须帮助读数但不能干扰主曲线。
- P5/P25/P50/P75/P95/MIN/MAX 使用竖向虚线标记，虚线从 x 轴开始，到对应抽数的 CDF 概率高度结束，即与曲线相交并绘制交点就结束；当前实现使用可见 dash（例如 `7 9`），不得退化为近似实线。
- MEAN 同时使用竖向和横向虚线标记，虚线从 x 和 y 轴开始，到 MEAN_LEVEL 和到 MEAN 结束，即与曲线相交并绘制交点就结束。
- MEAN 和 P50 常常会比较接近，文字标注参考 [模拟项目可视化](/src/simulate/visualize.py):593-626 行的做法避免重叠：普通分位标注优先放在交点左上方，MEAN 标注优先放在右下方，MAX 标注收在右上边界内。
- 文字标注必须做边界夹取和近邻错层，尽量避免与曲线、其它标注或图表边缘重叠。
- CDF 图中的标记虚线视觉权重需要区分，P50 的视觉权重最高，MAX/P95/MEAN 次之，P25/P75 更弱，MIN/P5最弱。
- 不允许为了美观改变 CDF 数据，完全按照输入数据绘制即可。

# 视觉风格
视觉方向为 **Sentry 深色监控台 + 少量 NVIDIA 科技终端细节**。页面应像高端工程监控 / 分析平台，而不是游戏 HUD 或营销页。

实际落地风格：

- 画布固定为深紫黑色网格背景，主背景 `#120d1f`，面板背景 `#1b142b`，次级面板 `#211936`。
- 文字使用深色界面上的高可读浅色：主文字 `#f7f3ff`，次级文字 `rgba(247, 243, 255, 0.68)`，弱提示文字 `rgba(247, 243, 255, 0.42)`。
- 卡片使用 6px 小圆角、细边框 `#362d59` / `#51436f` 和克制阴影，形成层次但不制造浮夸装饰。
- NVIDIA 绿色只用于少量角标、技术细线和分位语义色，不作为大面积背景。
- 禁止使用 Sentry 贴纸、吉祥物、大营销 hero，也不采用 NVIDIA 全黑 hero/footer 结构。
- 装饰只能服务信息层级：允许细网格、角标、小型状态标签和轻微 glow；不使用 orb、大片渐变、过度发光或战斗 UI 式装饰。

主曲线颜色固定为 Cyan `#22d3ee`，是画面中最强视觉信号。
...
CDF 标记颜色按照以下表格执行：
 ...
 | MEAN     |  #952fc6 | NVIDIA Accent Purple，平均值，避免重复使用终止原因 PK 条颜色 |
...
- 面板宽度约为主内容区 25%，必须紧凑，不能抢占 CDF 曲线注意力。
- 八个分布统计量按三组展示：较优结果、中心位置、尾部风险。
- 每个指标展示名称、简短说明、数值、单位；说明和数值使用该指标对应颜色。
- 单抽成本 `COST` 作为第九个核心统计量，以底部紧凑成本参考行展示，不参与分布风险分组。
- 分组标题使用真实 heading 语义，便于测试与可访问性定位。

终止原因 PK 条视觉：

- 终止原因颜色不表达好坏语义，固定使用语义中性但有对比的两色：Sentry Violet Link `#6a5fc1` 与 Sentry Hot Pink `#fa7faa`。
- 只有一种终止原因时，使用第一色 `#6a5fc1` 填满整条。
- 有两种终止原因时，按输入比例绘制两段，交界使用左斜 45° 斜切，不使用竖直分割线。
- 图例只表达颜色与原因的对应关系，不赋予颜色额外语义。

参考文档：[Sentry](./Sentry.md)、[NVIDIA](./NVIDIA.md)。
