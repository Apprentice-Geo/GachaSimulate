...
`src/visualize/` 是平台无关的结果可视化层。它把 `Analysis + DisplayConfig v2` 转换为经过校验的展示模型，供 Electron 展示和素材导出使用。导出宿主位于 `src/main/` 与 `src/export-renderer/`；这些宿主只依赖本层，不被本层反向依赖。

素材导出是长期保留能力。Electron 逐帧导出路线已经取代旧导出宿主；最终安装包在后续阶段单独验收。
...
导出能力是架构要求。Electron 展示与逐帧导出必须使用同一输入契约与共享逐帧语义；替换宿主不得复制或分叉画面逻辑。
...
桌面与导出共享紫黑底色、细边框和小方形节点的仪器台视觉语言：荧光绿用于品牌与主要操作，青色用于数据及选中反馈，普通标识符使用次要文字色。中文从 `fonts/SourceHanSansSC/OTF/SimplifiedChinese/` 加载 Regular、Medium、Bold；数字与标识符使用 `fonts/JetBrainsMono/fonts/webfonts/` 中的 JetBrains Mono Regular、SemiBold，关闭编程连字。数字字体由共享 token 管理，桌面与导出使用相同资源；单位继续使用思源黑体并降低字号与字重。字体目录保留原始授权材料，分发使用 `third_party/licenses/` 中的集中副本。

桌面运行模拟、结果编辑和配置仓库保留填满可用空间的布局；面板不可为减少留白而改为随内容收缩。配置仓库官方与本地分区按 7:3 分配剩余空间，少量内容时仍保留分区高度，长列表在各自内容区域内滚动。
...
- 修改画布规格或共享视觉 token 时，同时检查交互展示、Electron 导出 renderer、导出结果和相关文档。
...
- Electron MP4 路径使用固定源码构建的 Windows x64 FFmpeg，不得回退到 PATH，且当前不得进入安装包。发布边界和解除条件见 `docs/FFMPEG_DISTRIBUTION.md`。
