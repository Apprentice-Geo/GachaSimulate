...
function fail(path, message) { throw new CompilerError(path, message); }
...
function actions(value, path) {
    ...
    result.forEach((item, index) => { if (typeof item !== "string")
        fail(`${path}[${index}]`, "must be an action string"); });
    return result;
}
/** Compiles the v1 YAML contract to a JSON-serializable, flat arena IR. */
export function compile_yaml(config_text, termination_text, manifest_text) {
...
export function compile(configValue, terminationValue, manifestValue) {
    ...
    const strings = [];
    const stringId = (value) => { const found = strings.indexOf(value); if (found >= 0)
        return found; strings.push(value); return strings.length - 1; };
    const rawItems = list(config.items, "config.items");
    if (!rawItems.length)
        fail("config.items", "must be non-empty");
    const itemIds = new Map();
    const items = [{ id: stringId("draw_count"), name: stringId("Draw count") }];
    ...
    rawItems.forEach((entry, index) => {
        ...
        if (id === "draw_count")
            fail(`config.items[${index}]`, "draw_count is reserved");
        if (itemIds.has(id))
    ...
    const action = (value, path) => {
        ...
        if (item) {
            ...
            if (index === 0)
                fail(path, "draw_count is read-only");
            const amount = integer(Number(item[3]), path, item[2] !== "=");
            return { kind: item[2] === "+=" ? "add_item" : item[2] === "-=" ? "reduce_item" : "set_item", item: index, amount };
        }
    ...
    rawPools.forEach((entry, index) => { const single = map(entry, `config.pools[${index}]`); if (Object.keys(single).length !== 1)
        fail(`config.pools[${index}]`, "must be a single-key mapping"); const id = Object.keys(single)[0]; if (!ID.test(id) || poolIds.has(id))
        fail(`config.pools[${index}]`, "invalid or duplicate pool id"); poolIds.set(id, index); pools.push({ id: stringId(id), entries: { begin: 0, count: 0 } }); });
    rawPools.forEach((entry, index) => {
        ...
        let total = 0;
        let weighted;
        entries.forEach((raw, entryIndex) => { const path = `config.pools[${index}].${id}[${entryIndex}]`; const entry = map(raw, path); keys(entry, path, ["probability", "weight", "actions"]); const isWeight = "weight" in entry; if (("probability" in entry) === isWeight)
            fail(path, "must contain exactly one of probability or weight"); if (weighted != null && weighted !== isWeight)
            fail(path, "cannot mix probability and weight"); weighted = isWeight; total += number(entry[isWeight ? "weight" : "probability"], `${path}.${isWeight ? "weight" : "probability"}`); pool_entries.push({ threshold: total, actions: range(actions(entry.actions, `${path}.actions`), `${path}.actions`) }); });
        ...
        slice.forEach((entry) => { entry.threshold = weighted ? entry.threshold / total : entry.threshold; });
        slice.at(-1).threshold = 1;
        pools[index].entries = { begin, count: slice.length };
    });
    const conditions = [];
    const condition_children = [];
    const condition = (raw, path) => { const node = map(raw, path); const own = range(actions(node.actions, `${path}.actions`), `${path}.actions`); if ("check" in node) {
        ...
        return conditions.push({ kind: "check", item, op: match[2], value: integer(Number(match[3]), `${path}.check`), actions: own }) - 1;
    ...
    retained.forEach((raw, index) => { const value = map(raw, `termination.retained_items[${index}]`); if (Object.keys(value).length !== 1)
        fail(`termination.retained_items[${index}]`, "must be a single-key mapping"); const [id, count] = Object.entries(value)[0]; const item = itemIds.get(id); if (item == null || retainedIds.has(id))
        fail(`termination.retained_items[${index}]`, "unknown or duplicate retained item"); retainedIds.add(id); resolve[item].retain = Math.max(resolve[item].retain, integer(count, `termination.retained_items[${index}].${id}`)); });
    const everyPathHasAction = (raw, path) => { const node = map(raw, path); if (actions(node.actions, `${path}.actions`).length)
        return true; if ("check" in node)
        return false; const children = list(node.children, `${path}.children`); return ["OR", "||"].includes(node.op) ? children.every((child, index) => everyPathHasAction(child, `${path}.children[${index}]`)) : children.some((child, index) => everyPathHasAction(child, `${path}.children[${index}]`)); };
    const rules = [];
    const rawRules = config.rules == null ? [] : list(config.rules, "config.rules");
    rawRules.forEach((raw, index) => { const value = map(raw, `config.rules[${index}]`); if (Object.keys(value).length !== 1)
        fail(`config.rules[${index}]`, "must be a single-key mapping"); const [id, body] = Object.entries(value)[0]; const rule = map(body, `config.rules[${index}].${id}`); keys(rule, `config.rules[${index}].${id}`, ["mode", "condition"]); const mode = rule.mode ?? "once"; if (!["once", "per_draw", "repeat"].includes(mode))
        fail(`config.rules[${index}].${id}.mode`, "unsupported mode"); if (mode === "repeat" && !everyPathHasAction(rule.condition, `config.rules[${index}].${id}.condition`))
        fail(`config.rules[${index}].${id}.condition`, "every repeat path must contain at least one action"); rules.push({ id: stringId(id), mode, condition: condition(rule.condition, `config.rules[${index}].${id}.condition`) }); });
    const everyPathTerminates = (raw, path) => { const node = map(raw, path); if (actions(node.actions, `${path}.actions`).some((entry) => entry.trim().startsWith("terminate ")))
        return true; if ("check" in node)
        return false; const children = list(node.children, `${path}.children`); return ["OR", "||"].includes(node.op) ? children.every((child, index) => everyPathTerminates(child, `${path}.children[${index}]`)) : children.some((child, index) => everyPathTerminates(child, `${path}.children[${index}]`)); };
    ...
    const wantsCost = manifest != null && (manifest.metrics == null ? false : list(manifest.metrics, "manifest.metrics").some((metric, index) => {
        if (typeof metric !== "string")
            fail(`manifest.metrics[${index}]`, "must be a string");
        return metric === "cost";
    }));
    const costItem = itemIds.get("cost_count");
    if (wantsCost !== (costItem != null))
        fail("manifest.metrics", "cost requires exactly one config item named cost_count");
