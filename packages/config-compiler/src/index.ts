...
export function compile(configValue: unknown, terminationValue: unknown): CompiledProgram {
  ...
  resolves.forEach((raw, index) => { const path = `config.item_resolve[${index}]`; const value = map(raw, path); keys(value, path, ["item", "retain", "actions"]); const item = typeof value.item === "string" ? itemIds.get(value.item) : undefined; if (!item || resolve[item].actions.count) fail(`${path}.item`, "unknown, reserved, or duplicate resolved item"); const values = actions(value.actions, `${path}.actions`); if (!values.length || values.filter((entry) => new RegExp(`^${value.item}\\s*-=`).test(entry.trim())).length !== 1) fail(`${path}.actions`, "must contain exactly one reduce action for the resolved item"); resolve[item] = { retain: integer(value.retain, `${path}.retain`), actions: range(values, `${path}.actions`) }; });
